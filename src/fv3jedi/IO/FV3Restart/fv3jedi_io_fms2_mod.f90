  ...
  do f = 1, size(fields)
    ...
    if (trim(fields(f)%long_name) /= trim(cached_field_names(f))) then
      ...
      if(rank==0) write(6,'("read_restart_fields_reg: field name order is different",I4,5A)') &
                        f,' (', trim(fields(f)%long_name),') /= (', trim(cached_field_names(f)),')'
      exit
    endif
...
if( (fields_changed) .or. &
    (globalsizes(1) .ne. cached_globalsizes(1)) .or. &
    (globalsizes(2) .ne. cached_globalsizes(2)) ) then
  ...
  cached_globalsizes = globalsizes
  ...
  do jedi_var_idx = 1,size(fields)
    ...
    if (len_trim(tmpvarlist(indexrst)) > 0) then
      tmpvarlist(indexrst)=trim(tmpvarlist(indexrst)) //' '// ioname(trim(fields(jedi_var_idx)%long_name), field_io_names_local)
    else
      tmpvarlist(indexrst)=ioname(trim(fields(jedi_var_idx)%long_name), field_io_names_local)
    endif
  ...
  do jedi_var_idx = 1,size(fields)
    do file_var_idx = 1,sum(numvar)
      if (ioname(trim(fields(jedi_var_idx)%long_name), field_io_names_local) == trim(varnames(file_var_idx))) then
  ...
  call TwoPhaseScatter_init(geom, npes, batch_size)
...
! read 2D slab from each file using sub communicator
  ...
  if (MPI_COMM_NULL /= read_comm) then
    ...
    if(mype_vartype==NF90_FLOAT) then
      allocate(d3r4(globalsizes(1),globalsizes(2),mype_lbegin:mype_lend))
    elseif(mype_vartype==NF90_DOUBLE .and. kind_real == c_double) then
      ...
      allocate(d3r8(globalsizes(1),globalsizes(2),mype_lbegin:mype_lend))
    ...
    do ilev=mype_lbegin,mype_lend
      starts=(/1,1,ilev,1/)
      counts=(/globalsizes(1), globalsizes(2), 1, 1/)
...
! ------------------------------------------------------------------------
  ...
  do b_start = 1, ntotallev, batch_size
    ...
    do b_ind = 1, n_in_batch
      ...
      if(rank == owner) then
        ...
        if(nc_vartype(file_var_idx) == NF90_FLOAT) then
          call TwoPhaseScatter_Phase1(owner, rank, d3r4(:,:,l), b_ind, reqs_p1(b_ind))
        elseif(nc_vartype(file_var_idx) == NF90_DOUBLE .and. kind_real == c_double) then
          call TwoPhaseScatter_Phase1(owner, rank, d3r8(:,:,l), b_ind, reqs_p1(b_ind))
        ...
        if(nc_vartype(file_var_idx) == NF90_FLOAT) then
          call TwoPhaseScatter_Phase1(owner, rank, dummy_r4, b_ind, reqs_p1(b_ind))
        elseif(nc_vartype(file_var_idx) == NF90_DOUBLE .and. kind_real == c_double) then
          call TwoPhaseScatter_Phase1(owner, rank, dummy_r8, b_ind, reqs_p1(b_ind))
        elseif(nc_vartype(file_var_idx) == NF90_DOUBLE .and. kind_real == c_float) then
          call TwoPhaseScatter_Phase1(owner, rank, dummy_r4, b_ind, reqs_p1(b_ind))
        endif
      endif
    ...
    do b_ind = 1, n_in_batch
      ...
      if(nc_vartype(file_var_idx) == NF90_FLOAT .and. kind_real == c_double) then
        ...
        call TwoPhaseScatter_Phase2(owner, rank, scatter_recv_cast_workspace_r4(:,:,b_ind), b_ind, reqs_p2(b_ind))
      else
        ! Scatter directly to strictly typed application array
        call TwoPhaseScatter_Phase2(owner, rank, fields(jedi_var_idx)%array(:,:,LevelToLevelMap(level)), &
                                    b_ind, reqs_p2(b_ind))
      endif
    end do
  ...
  !te3 = MPI_Wtime()
  !times(3) = te3-tb3
  !call MPI_Reduce(times, walltime, 3, MPI_DOUBLE_PRECISION, MPI_MAX, 0, %f_comm%communicator(), ierr)
...
if( (fields_changed) .or. &
  ...
  do jedi_var_idx = 1,size(fields)
    ...
    ! Append variable name onto list for each file.  Will need a mapping between this list and the order in the fields array
    if (len_trim(tmpvarlist(indexrst)) > 0) then
      tmpvarlist(indexrst)=trim(tmpvarlist(indexrst)) //' '// ioname(trim(fields(jedi_var_idx)%long_name), field_io_names)
    else
      tmpvarlist(indexrst)=ioname(trim(fields(jedi_var_idx)%long_name), field_io_names)
    endif
  ...
  if(all(file_exists(1:totalnumfiles) .eqv. .true.)) then
    ...
    do n = 1, numfiles
      ...
      do k = 1, num_restart_vars(n)
        ...
        tmp_names(i) = ioname(trim(fields(jedi_var_idx)%long_name), field_io_names)
  ...
  do jedi_var_idx = 1, size(fields)
    do file_var_idx = 1, sum(numvarfile)
      if (ioname(trim(fields(jedi_var_idx)%long_name), field_io_names) == trim(varnames(file_var_idx))) then
        VarToVarMap(file_var_idx) = jedi_var_idx
        exit
      endif
    enddo
  ...
  if (MPI_COMM_NULL /= write_comm) then
    ...
    do file_var_idx = 1,sum(numvarfile)
      ...
      if (nc_vartype(file_var_idx) == NF90_FLOAT) then
        allocate(real(kind=c_float)::write_buffers(jedi_var_idx)%r4(globalsizes(1),globalsizes(2),mype_lbegin:mype_lend))
      else
        allocate(real(kind=c_double)::write_buffers(jedi_var_idx)%r8(globalsizes(1),globalsizes(2),mype_lbegin:mype_lend))
      endif
    enddo
  endif ! write_comm
  !te = MPI_Wtime()
  ...
  call TwoPhaseGather_init(geom, npes, batch_size)
...
do b_start = 1, ntotallev, batch_size
  ...
  do b_ind = 1, n_in_batch
    ...
    if (nc_vartype(file_var_idx) == NF90_FLOAT .and. kind_real == c_double) then
      ...
      call TwoPhaseGather_Phase1(owner, rank, gather_send_cast_workspace_r4(:,:,b_ind), b_ind, reqs_p1(b_ind))
    else
      ! Send the native application type (no downcast needed)
      call TwoPhaseGather_Phase1(owner, rank, fields(jedi_var_idx)%array(:,:,LevelToLevelMap(level)), b_ind, reqs_p1(b_ind))
    endif
  end do
  ...
  do b_ind = 1, n_in_batch
    ...
    if(rank == owner) then
      if (nc_vartype(file_var_idx) == NF90_FLOAT) then
        call TwoPhaseGather_Phase2(owner, rank, write_buffers(jedi_var_idx)%r4(:,:,l), b_ind, reqs_p2(b_ind))
      else
        call TwoPhaseGather_Phase2(owner, rank, write_buffers(jedi_var_idx)%r8(:,:,l), b_ind, reqs_p2(b_ind))
      endif
      l=l+1
    else
      ! Non-owners hit MPI_BOTTOM via dummies routed by kind_real
      if (nc_vartype(file_var_idx) == NF90_FLOAT) then
        call TwoPhaseGather_Phase2(owner, rank, dummy_r4, b_ind, reqs_p2(b_ind))
      else
        call TwoPhaseGather_Phase2(owner, rank, dummy_r8, b_ind, reqs_p2(b_ind))
      endif
    endif
  end do
    ...
    if (self%write_into_existing_files) then
      ...
      if(rc == nf90_noerr) then
        ...
        do file_var_idx = b, e
          ...
          !write(6,'("Checksum for jedi_var_idx "5A)') ioname(trim(fields(jedi_var_idx)%long_name), field_io_names),' ',trim(chksum),' ',trim(varnames(file_var_idx))
      ...
      if(rc == nf90_noerr) then
        ...
        call check( nf90_def_dim(ncid(n), 'xaxis_1', globalsizes(1), dimids(1)) )
        call check( nf90_def_dim(ncid(n), trim(ydim_name), globalsizes(2), dimids(2)) )
        ...
        call check( nf90_def_var(ncid(n), trim(ydim_name), NF90_DOUBLE, dimids(2), varid) )
        call check( nf90_put_att(ncid(n), varid, "cartesian_axis", "Y") )
        call check( nf90_put_att(ncid(n), varid, "long_name", trim(ydim_name)) )
        call check( nf90_put_att(ncid(n), varid, "units", "none") )
      ...
      do file_var_idx = b, e
        ...
        if(nlevpervar(file_var_idx) > 1) then
          chunksizes = [globalsizes(1), globalsizes(2), 1, 1]
          if (nc_vartype(file_var_idx) == NF90_FLOAT) then
          ...
          chunksizes = [globalsizes(1), globalsizes(2), 1]
          if (nc_vartype(file_var_idx) == NF90_FLOAT) then
...
! ALL ranks in write_comm MUST loop over every variable in the file together!
  do file_var_idx = b, e
    ...
    if (file_var_idx == my_var_index) then
      ...
      countloc = (/ globalsizes(1), globalsizes(2), (mype_lend - mype_lbegin + 1), 1 /)
